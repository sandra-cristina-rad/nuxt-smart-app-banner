...
import {
  SmartAppBannerNotShownReason,
  SmartAppBannerPlatform,
  type SmartBannerOptions,
} from "../types";
import {
  identifyPlatform,
  isMobileSafariPlatform,
  platformsData,
} from "../platformHelper";
...
const useSmartAppBannerCallbacks = (
  appId: ComputedRef<string>,
  platform: Ref<SmartAppBannerPlatform>,
  setShowBanner: (value: boolean) => void,
  emit: any,
  bannerConfig: SmartBannerOptions
) => {
  const installClick = () => {
    setShowBanner(false);
    Cookies.set(appId.value + "-smartbanner-installed", "true", {
      path: "/",
      expires: new Date(
        Number(new Date()) + bannerConfig.daysReminder * 1000 * 60 * 60 * 24
      ),
    });
  ...
  const dismissClick = () => {
    ...
    Cookies.set(appId.value + "-smartbanner-closed", "true", {
      path: "/",
      expires: new Date(
        Number(new Date()) + bannerConfig.daysHidden * 1000 * 60 * 60 * 24
      ),
    });
...
const useSmartAppBannerState = (bannerConfig: SmartBannerOptions) => {
  ...
  const appId = computed(() =>
    platformsData[platform.value].getAppId(bannerConfig)
  );
  const storeLink = computed(() =>
    platformsData[platform.value].getStoreLink(bannerConfig)
  );
  const theme = computed(() => bannerConfig.theme || platform.value);
  const inStoreText = computed(
    () =>
      bannerConfig.price[platform.value] +
      " - " +
      bannerConfig.store[platform.value]
  );
  const icon = computed(() => {
  ...
  const mainContainerClass = computed(
    () => `smartbanner smartbanner-${theme.value}`
  );
...
export const useSmartAppBanner = (emit: any) => {
  const bannerConfig = useRuntimeConfig().public
    .smartAppBanner as SmartBannerOptions;
  const {
    appId,
    inStoreText,
    platform,
  ...
  const { dismissClick, installClick } = useSmartAppBannerCallbacks(
    appId,
    platform,
    setShowBanner,
    emit,
  ...
  const state = {
    ...
    buttonText: bannerConfig.button,
    showBanner,
    storeLink,
    inStoreText,
    dismissClick,
      ...
      meta: [
        {
          ...
          content: `app-id=${bannerConfig.iosAppId}`,
        },
      ],
    });
  }
  ...
  const isMobileSafari =
    bannerConfig.useNativeIosBannerForSafari &&
    isMobileSafariPlatform(platform.value, bowser);
  ...
  if (userDismissed) {
    emit(
      ...
      platform.value,
      appId.value,
      SmartAppBannerNotShownReason.dismissed
    );
    return state;
  }
  ...
  if (userInstalled) {
    emit(
      ...
      platform.value,
      appId.value,
      SmartAppBannerNotShownReason.installed
    );
    return state;
  }

  // - If we don't have app id, don't display the banner
  if (!appId.value) {
    return state;
  }
